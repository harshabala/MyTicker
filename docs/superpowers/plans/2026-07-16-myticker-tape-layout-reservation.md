...
**Architecture:** Extend the content-script lifecycle with a small reservation manager. It owns inline body/root values and a scoped ChatGPT image-dialog offset; tape height comes from measurement and changes are observed, not inferred from a visual preset.
...
- [ ] Implement a selector-scoped ChatGPT adapter that adjusts only an open full-screen `<dialog>` for generated-image viewing. Do not use root transforms, a normal-page `<main>` offset, or fixed-element enumeration.
