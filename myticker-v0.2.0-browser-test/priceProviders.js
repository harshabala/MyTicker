...
export class FinnhubPriceProvider {
  ...
  async _fetchSingle(symbol, baseUrl, apiKey) {
    // Allowlist baseUrl — prevents a tampered sync config from redirecting the API key.
    const safeBase = String(baseUrl || "").startsWith("https://finnhub.io")
      ? baseUrl
      : "https://finnhub.io/api/v1";
    const url = `${safeBase}/quote?symbol=${encodeURIComponent(symbol)}&token=${encodeURIComponent(apiKey)}`;
