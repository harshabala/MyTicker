...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  // Accept messages only from this extension's own pages (popup, options).
...
async function handlePricePoll() {
  ...
  try {
    ...
    // API key is stored only in local storage (never sync) — ignore any stale sync fallback.
    const apiConfig = {
      ...(settings.priceProviderConfig || {}),
      apiKey: localData["pts_price_api_key"] || ""
    };
    if (!apiConfig.apiKey) {
