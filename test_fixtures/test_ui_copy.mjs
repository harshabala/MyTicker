...
assert(goldThemes.every(([, tokens]) => ["green", "bg-surface"].every((name) => tokens[name]) && contrastRatio(tokens.green, tokens["bg-surface"]) >= 4.5), "keeps the semantic success token at WCAG AA contrast on every checklist surface");
...
assert(/\.checklist-item\.done \.check-icon\s*\{[\s\S]*?background:\s*color-mix\(in srgb, var\(--green\) 18%, transparent\)[\s\S]*?color:\s*var\(--green\)/.test(popupHtml), "keeps completed popup checklist steps in the semantic success color");
